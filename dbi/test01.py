...
def find_movie(list, fn_input):
    if list == '0':
		list3 = i.search_movie(fn_input)
    else:
      	set1 = set(list1)
        set2 = set(list2)
        set3 = set1 & set2
    ...
    if len(list3) > 0:
        ...
        while count < max_count:
            ...
            count = count + 1
        user_input = raw_input("Want any of these(1-5), if not (6).  ")
        if user_input < '6':
...
''' ----------------------------------------------        
    ...
    if len(result) > 0:
        ...
        max_count = min(length,5)
        while count < max_count:
            print count + 1, '->', result[count]['long imdb title'], ' ', len(result[count]), ' ', len(result)
            count = count + 1
        user_input = raw_input("Want any of these(1-5), if not (6).  ")
        if user_input < '5':
...
while count is count:
    ...
    if len(result) > 1:
    	print len(result)
    	raw_input("Good, we got to this location Jordan")
    else:
    	print len(result)
    	raw_input("You specified a specific instance right?")